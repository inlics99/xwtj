		...
		<TBODY>
			...
			<TR>
				<TD  class=middle align="left" >
                    ...
                    <%
                    String ipaddress=request.getRemoteAddr();//获取本机ip地址
                    String member=(String)session.getAttribute("member");//获取用户名
                    if(member==null)member=ipaddress; //如果没有用户登录就用本机ip地址作代替
                    String sql="select count(*) from keyword where member='"+member+"'  ";//获取推荐新闻总数
                    String sql2="select * from keyword where member='"+member+"' order by id desc";//获取推荐新闻数据
                    System.out.println(sql2);
                    news.setEVERYPAGENUM(12);//每页显示12条数据
					...
					int pageCount = news.getPageCount(); //得到页码总数  
