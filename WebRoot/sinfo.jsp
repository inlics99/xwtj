			...
			<TD  class=middle align="left" >
               <TABLE class=xsnr id=tb1_nr1  cellSpacing=0 cellPadding=0 width="100%" border=0>
                   ...
                   <tr class="head">  
                    ...
                    //comServiceImp.comUp2(String) 收集用户的搜索记录获取搜索关键词,方便推送同类型新闻,没有登录就记录本机ip
                    cb.comUp2("insert into keyword(fenlei,title,member,sj) values('"+fenlei+"','"+title+"','"+member+"','"+date2+"'  )");
                    String sql="select count(*) from fz_news where fenlei='"+fenlei+"' and gjz like '%"+title+"%' ";
                    String sql2="select * from fz_news where fenlei='"+fenlei+"' and gjz like '%"+title+"%' order by id desc";
                    /* System.out.println(sql2); */
                    news.setEVERYPAGENUM(12);//设置每页显示12条数据
					...
					//newsServiceImpl.getMessageSS(int, String, int) 查询当前页码(page1)的内容
					List pagelist1 = news.getMessageSS(Integer.parseInt(page1),sql2,8);
					session.setAttribute("qqq", pagelist1);
					int pageCount = news.getPageCount(); //得到页数总数  
					session.setAttribute("busPageCount", pageCount + ""); //发送到作用域,方便下面的操作取值
